...
public class BasaltTurretRenderer extends MobRenderer<BasaltTurretEntity, ModelBasalt_Turret<BasaltTurretEntity>> {
	private static final ResourceLocation beamtexture = new ResourceLocation("too_much_bosses:textures/entities/glowstone_beam.png");
	private static final RenderType BEAM_RENDER_TYPE = RenderType.entityCutoutNoCull(beamtexture);
	public BasaltTurretRenderer(EntityRendererProvider.Context context) {
		super(context, new ModelBasalt_Turret(context.bakeLayer(ModelBasalt_Turret.LAYER_LOCATION)), 0f);
		this.addLayer(new EyesLayer<BasaltTurretEntity, ModelBasalt_Turret<BasaltTurretEntity>>(this) {
			@Override
			public RenderType renderType() {
				return RenderType.eyes(new ResourceLocation("too_much_bosses:textures/entities/basaltturret_glowlayer.png"));
			}
		});
	}

	public boolean shouldRender(BasaltTurretEntity p_114836_, Frustum p_114837_, double p_114838_, double p_114839_, double p_114840_) {
	  return true;
   ...
   @Override
	public void render(BasaltTurretEntity entityIn, float entityYaw, float partialTicks, PoseStack poseStack, MultiBufferSource bufferIn, int packedLightIn) {
		...
		//BasaltTurretRenderer.setupRotations(entityIn, poseStack, 0f, 0f, partialTicks);
		 ...
		 poseStack.popPose();*/
		 if(TooMuchBossesModVariables.MapVariables.get(world).isShooting == true){
         ...
         Vec3 vec3 = new Vec3(/*entityIn.shooteX,entityIn.shooteY,entityIn.shooteZ);*/(double)(TooMuchBossesModVariables.MapVariables.get(world).shootX)/*entityIn.shootX*/,(double)(TooMuchBossesModVariables.MapVariables.get(world).shootY)/*entityIn.shootY*/,(double)(TooMuchBossesModVariables.MapVariables.get(world).shootZ)/*entityIn.shootZ*/); //= this.getPosition((Entity) livingentity, (double)livingentity.getBbHeight() * 0.5D, partialTicks);
         Vec3 vec31 = new Vec3(entityIn.getX(),entityIn.getY() + 0.9D,entityIn.getZ());//this.getPosition(((Entity) entityIn), (double)f3, partialTicks);
         ...
         float f8 = f * f;
         int j = 255;
         int k = 255 - ((int) Math.random() * 10);//+ (int)(entityIn.lifeStealTicks) * 6;
         int l = 0;
         float f9 = 0.2F;
         float f10 = 0.282F;
   ...
   private static void vertex(VertexConsumer p_114842_, Matrix4f p_114843_, Matrix3f p_114844_, float p_114845_, float p_114846_, float p_114847_, int p_114848_, int p_114849_, int p_114850_, float p_114851_, float p_114852_) {
	...
	protected void scale(BasaltTurretEntity p_114046_, PoseStack p_114047_, float p_114048_) {
		p_114047_.scale(1.0f, 1.0f, 1.0f);
	}

	protected void setupRotations(BasaltTurretEntity p_115317_, PoseStack p_115318_, float p_115319_, float p_115320_, float p_115321_) {
   }
   
	@Override
	public ResourceLocation getTextureLocation(BasaltTurretEntity entity) {
		if(entity.getFungus() == 1){
			return new ResourceLocation("too_much_bosses:textures/entities/basalt_turret_1.png");
		}
		return new ResourceLocation("too_much_bosses:textures/entities/basalt_turret_2.png");
	}
}
