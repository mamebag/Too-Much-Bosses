...
public class BGflameEffectsProcedure {
	@SubscribeEvent
	...
	private static void execute(@Nullable Event event, LevelAccessor world, Entity entity) {
		...
		double b = 0;
		double dis = 0;
		if (entity instanceof BGFlameEntityEntity) {
			r = 0.5;
			a = Math.random() * 12;
			b = 90;
			for (int index0 = 0; index0 < 30; index0++) {
				for (int index1 = 0; index1 < 16; index1++) {
					...
					world.addParticle(ParticleTypes.FLAME, (entity.getX()), (entity.getY() + 1), (entity.getZ()), ((((entity.getX() - r * Math.cos(Math.toRadians(b)) * Math.sin(Math.toRadians(a))) - entity.getX()) / dis) / 3),
							((((entity.getY() - r * Math.sin(Math.toRadians(b))) - entity.getY()) / dis) / 3), ((((entity.getZ() + r * Math.cos(Math.toRadians(b)) * Math.cos(Math.toRadians(a))) - entity.getZ()) / dis) / 3));
				}
				b = 90;
				a = a + 12;
			}
